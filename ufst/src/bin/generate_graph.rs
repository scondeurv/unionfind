...
fn main() {
    ...
    if args.len() < 4 {
        eprintln!("Usage: {} <output_file> <num_nodes> <num_edges> [num_components] [--binary]", args[0]);
        eprintln!("");
        eprintln!("Arguments:");
        eprintln!("  output_file     Output file path");
        ...
        eprintln!("  --binary        Write binary file (u32 src + u32 dst, little-endian)");
        std::process::exit(1);
    }
    ...
    for arg in args.iter().skip(4) {
        if arg == "--binary" {
            ...
            num_components = arg.parse().expect("Invalid num_components");
        }
    }

    eprintln!(
        "Generating graph with {} nodes, {} edges, ~{} components ({})",
        num_nodes,
        num_edges,
        num_components,
        if binary_mode { "binary" } else { "tsv" }
    );
    ...
    for c in 0..num_components {
        ...
        for node in (start + 1)..end {
            ...
            if binary_mode {
                writer.write_all(&node.to_le_bytes()).unwrap();
                writer.write_all(&target.to_le_bytes()).unwrap();
            } else {
        ...
        for _ in 0..edges_per_component {
            ...
            if src != dst {
                if binary_mode {
                    ...
                    writer.write_all(&dst.to_le_bytes()).unwrap();
                } else {
